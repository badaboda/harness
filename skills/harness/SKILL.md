...
> ⚠️ **팀 실행 방식 변경 (Claude Code 2026-07+):** `TeamCreate`/`TeamDelete`는 없어졌다. 팀원은 **`Agent` 도구를 `name`과 함께 호출**해 스폰하고(세션이 곧 단일 암묵 팀), `SendMessage({to:name})` + 공유 `TaskCreate`로 조율한다. 동시 실행은 한 메시지에 여러 `Agent` 호출. 구체 패턴·코드·마이그레이션은 **`references/team-mode-mechanism.md` 필독**.
>
...
- **팀 모드 실행 메커니즘 (Claude Code 2026-07+, TeamCreate 폐지)**: `references/team-mode-mechanism.md` — 팀을 구성·실행하기 전 필독
